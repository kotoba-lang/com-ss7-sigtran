(ns ss7_sigtran.main-test
  "Contract + behavioral test for the ss7_sigtran-compat L5 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ss7_sigtran-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ss7_sigtran.main-test)]
