(ns ss7_sigtran.main
  "Kotodama WASM entrypoint for the Ss7_sigtran clean-room actor (L5) — Clojure port.
  ...
  facts (`ss7_sigtran.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ss7_sigtran")
(def tier "L5")
...
(def entity-specs
  [{:entity "M3uaMessage"   :plural "m3uamessages"   :id-prefix "ss7sigtr_m3u"
    :fields [:version :reserved :messageClass :messageType :messageLength] :required [:version :reserved]
    :coerce {:version :int :reserved :int :messageClass :int :messageType :int :messageLength :int} :refs {}}
   {:entity "SuaMessage"    :plural "suamessages"    :id-prefix "ss7sigtr_sua"
    :fields [:version :messageClass :messageType :messageLength] :required [:version :messageClass]
    :coerce {:version :int :messageClass :int :messageType :int :messageLength :int} :refs {}}
   {:entity "ProtocolData"  :plural "protocoldatas"  :id-prefix "ss7sigtr_pro"
    :fields [:opc :dpc :si :ni :mp :sls] :required [:opc :dpc]
    :coerce {:opc :int :dpc :int :si :int :ni :int :mp :int :sls :int} :refs {}}
   {:entity "Mtp3Message"   :plural "mtp3messages"   :id-prefix "ss7sigtr_mtp"
    :fields [:serviceIndicator :networkIndicator :payload] :required [:serviceIndicator :networkIndicator]
    :coerce {:serviceIndicator :int :networkIndicator :int} :refs {}}
   {:entity "SccpMessage"   :plural "sccpmessages"   :id-prefix "ss7sigtr_scc"
    :fields [:messageType :destinationPointCode :sourcePointCode :subsystemNumber :globalTitle] :required [:messageType :destinationPointCode]
    :coerce {:messageType :int :destinationPointCode :int :sourcePointCode :int :subsystemNumber :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ss7_sigtran.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ss7_sigtran-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ss7_sigtran-compat/ready)
